...
- **AI 선생님** (설정 탭에서 켬): 학습 화면 "🤖 묻기", 문법 상세 "🤖 묻기", 단어 풍선의 "🤖 묻기"로 지금 보고 있는 단어·예문·문법을 자동으로 함께 보내며 LLM에게 질문. 빠른 질문 버튼(뜻·용법, 비슷한 단어 차이, 예문 더, 문법 분석, 외우는 요령, 흔한 실수 …) + 자유 질문. 제공자는 Gemini(무료 티어) · Claude · OpenAI 호환(DeepSeek, OpenAI 등 — API 주소와 모델 이름만 바꾸면 됨)을 고를 수 있고, API 키는 이 기기의 브라우저에만 저장(동기화·내보내기 제외). 답변은 단어별로 최근 8개까지 저장돼 같은 질문에 토큰을 다시 쓰지 않음. 서버 없이 브라우저가 각 회사 API를 직접 호출
